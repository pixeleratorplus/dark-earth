...
function drawThreeGeo(json, radius, shape, materalOptions, container) {
  ...
  for (let geom_num = 0; geom_num < json_geom.length; geom_num++) {
    if (json_geom[geom_num].type === 'Point') {
      ...
      for (let segment_num = 0; segment_num < json_geom[geom_num].coordinates.length; segment_num++) {
        ...
        // drawLine(x_values, y_values, z_values, materalOptions)
        // drawMesh(x_values, y_values, z_values, materalOptions)
        drawLineMesh(x_values, y_values, z_values, materalOptions)
      }
  ...
  function convertToSphereCoords(coordinates_array, sphere_radius) {
    ...
    y_values.push(Math.sin((lat * Math.PI) / 180) * sphere_radius)
    z_values.push(-Math.cos((lat * Math.PI) / 180) * Math.sin((lon * Math.PI) / 180) * sphere_radius)
  }
  ...
  function drawParticle(x, y, z, options) {
    ...
    const particle_geom = new THREE.BufferGeometry().setFromPoints(vertices)

    const particle_material = new THREE.ParticleSystemMaterial(options)

    const particle = new THREE.ParticleSystem(particle_geom, particle_material)
  ...
  function drawMesh(x_values, y_values, z_values, options) {
    const mesh_geom = new THREE.BufferGeometry()
    createVertexForEachPoint(mesh_geom, x_values, y_values, z_values)

    const mesh_material = new THREE.MeshBasicMaterial(options)
    const mesh = new THREE.Mesh(mesh_geom, mesh_material)
  ...
  function drawLineMesh(x_values, y_values, z_values, options) {
    const line_geom = new THREE.BufferGeometry()
    createVertexForEachPoint(line_geom, x_values, y_values, z_values)
  ...
  function createVertexForEachPoint(object_geometry, values_axis1, values_axis2, values_axis3) {
    ...
    for (let i = 0; i < values_axis1.length; i++) {
      vertices.push(new THREE.Vector3(values_axis1[i], values_axis2[i], values_axis3[i]))
    }
    object_geometry.setFromPoints(vertices)
